...
public class Main {
    ...
    static class JavaVersionOptionMixin {
        ...
        void setJavaVersion(String versionOrId) {
            if (!versionOrId.matches("\\d+[+!]?")) {
                throw new IllegalArgumentException(
                        "Invalid version, should be a number optionally followed by an exclamation mark");
            }
        ...
        String getVersionOrId(boolean quiet) {
            try {
                return Main.getVersionOrId(versionOrId);
            } catch (IOException e) {
                if (!quiet) {
    ...
    private static String validateVersionOrId(String versionOrId) {
        ...
        if (v.endsWith("+") && isInteger(v.substring(0, v.length() - 1))) {
            // Nothing to do
        } else if (v.endsWith("!") && isInteger(v.substring(0, v.length() - 1))) {
            parts[0] = v.substring(0, v.length() - 1);
        } else if (isInteger(v)) {
            parts[0] = v + "+";
